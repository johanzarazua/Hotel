    ...
    try{
        ...
        $exito_q2 = $db->query($query2);
            ...
            if($exito_q2->fetchColumn() >= 1){
                ...
                $query4 = "INSERT INTO `empleado`(`idEmpleado`,`Puesto`, `Usuario`, `Contraseña`, `Pregunta_seguridad`, `Respuesta_seguridad`) VALUES ((SELECT idPersona FROM persona ORDER BY idPersona DESC LIMIT 1),?,REPLACE(?,' ',''),MD5(?),?,TRIM(?))";
...
function empleado_ingresar($usuario, $contrasena){
    ...
    try{
        ...
        $query = "SELECT COUNT(*) FROM empleado WHERE Usuario = REPLACE('". $usuario ."',' ','') AND Contraseña = MD5('". $contrasena ."') AND Estado = 1";
...
function empleado_cambiar_contrasena($usuario, $contrasena){
    ...
    try{
        ...
        $query = "UPDATE empleado SET Contraseña = MD5('". $contrasena ."') WHERE Usuario = REPLACE('". $usuario ."', ' ','') AND Estado = 1";
